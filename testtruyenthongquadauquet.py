...
class POSDevice:
    ...
    def connect(self):
        self.connected = True
        print("POS device connected.")

    def scan_card(self, card_info):
        if self.connected:
            print(f"Scanning card: {card_info}")
            # Logic to send card_info to POS device
            # ...
            print("Card information transmitted to POS device.")
        else:
            print("POS device not connected.")
...
# Connect to POS device and scan selected card
pos_device = POSDevice()
pos_device.connect()

if selected_card:
    pos_device.scan_card(selected_card)
